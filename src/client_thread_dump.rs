...
pub fn client_thread(json_file_path: String, server_cid: u32, server_port: u32) -> Result<()> {
    ...
    let mut stream = match VsockStream::connect(&addr) {
        ...
        Err(e) => {
            ...
            return Err(Error::msg("Connection failed"));
        }
    };
    ...
    match stream.write_all(message.as_bytes()) {
        ...
        Err(e) => {
            ...
            return Err(Error::msg("Send error"));
        }
    }
     ...
     * 最后，服务端传输完毕，发送 "find-<count>" 标识，关闭；客户端收到 "find-<count>" 后，关闭连接。
     * 
     * 无消息:
     * 服务端，发送 "nofind" 标识，关闭；客户端收到 "nofind" 后，关闭连接。
     */

    fs::write(&json_file_path, "")?;    // 先清空文件内容
    ...
    loop {
        match stream.read(&mut buffer) {
            ...
            Ok(n) => {
                let recv_str = String::from_utf8_lossy(&buffer[..n]);
                let recv_str = recv_str.trim();

                if recv_str == "nofind" {
                    println!("[Client-Thread-For-Dump] Server reported no messages.");
                    break;
                } else if recv_str.starts_with("find-") {
                    let count_str = recv_str.trim_start_matches("find-");
                    println!("[Client-Thread-For-Dump] Server finished sending. Total count: {}", count_str);
                    break;
                } else {
                    // 尝试解析为 JSON
                    match serde_json::from_str::<serde_json::Value>(recv_str) {
                        ...
                        Err(e) => {
                             eprintln!("[Client-Thread-For-Dump] ✗ Failed to parse JSON: {:?}. Raw: {}", e, recv_str);
                             // 即使解析失败也发送 OK 以继续
            ...
            Err(e) => {
                ...
                return Err(Error::msg("Recv error"));
            }
        }
    }
    ...
    if !received_items.is_empty() {
        let pretty_json = serde_json::to_string_pretty(&received_items)?;
        fs::write(&json_file_path, pretty_json)?;
